...
ErrorMessageOut = game:GetService("LogService").MessageOut:Connect(function(Message, Type)
    ...
    if Type == Enum.MessageType.MessageError and not string.find(Message, "attempt to index nil with 'Value'") then
        ...
        Library:Library:Notify" NexusHack has errored while loading and will now unload. The error has been copied to your clipboard, please report this on the NH discord server! ", 4.5)
...
function Library:NotifyTitleText, SubText, Duration, Force)
    if not Force then
        ...
        elseif v:IsA("Model") then
            if v.Name == "GloombatSwarm" then
                ...
                if Toggles.GN_Entities.Value and Options.GN_Entities_Options.Value["Gloombat Swarm"] then
                    Library:Notifyv:GetAttribute("WaitForOpen") and "A gloombat swarm will occur in the next few rooms." or "A gloombat swarm has spawned!", "Keep off all light sources and keep going!")
                end
