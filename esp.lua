...
-- 1. ЗАГРУЗКА НОВОЙ БИБЛИОТЕКИ (MS-ESP .luau)
...
local Window = Library:CreateWindow({
    Title = "NexusHack ESP ┃ MS-ESP v3",
    Footer = "Fix: Highlight -> Add",
    Icon = 95816097006870,
    NotifySide = "Right",
...
function Script.Functions.ESP(args)
    ...
    -- ИСПРАВЛЕНИЕ: Используем ESPLibrary:Add вместо ESPLibrary.Highlight
    local highlight = ESPLibrary:Add({
        Name = ESPManager.Text,
        Model = ESPManager.Object,
        StudsOffset = ESPManager.Offset,

        -- Настройки Highlight (стандартные)
        ESPType = "Highlight", 
        
        FillColor = ESPManager.Color,
        OutlineColor = ESPManager.Color,
        -- Цвет текста для BillboardGui (берется из Color, так как TextColor нет в аргументах Add напрямую для Highlight, но есть Color)
    ...
    if not Script.ESPTable[args.Type] then Script.ESPTable[args.Type] = {} end
...
function Script.Functions.DoorESP(room)
    ...
    if door then
        ...
        -- Для новой библиотеки обновление текста может отличаться, но обычно работает через апдейт настроек или пересоздание
        -- В mspaint v2.0.4 нет прямого метода SetText в объекте, но мы можем обновить настройки
        Script.Connections[room.Name .. "Opened"] = door:GetAttributeChangedSignal("Opened"):Connect(function()
            if doorEsp then 
                -- Обновляем имя через :Add снова или меняем поле в CurrentSettings, если доступно
                -- В новой версии надежнее всего просто обновить Name в настройках
                if doorEsp.CurrentSettings then
                    doorEsp.CurrentSettings.Name = string.format("Door %s [Opened]", doorNumber)
                end
            end
        end)
    end
...
function Script.Functions.PlayerESP(player)
    ...
    Script.Connections[player.Name .. "HP"] = player.Character.Humanoid.HealthChanged:Connect(function(newHealth)
        if newHealth > 0 then
            if playerEsp and playerEsp.CurrentSettings then 
                playerEsp.CurrentSettings.Name = string.format("%s [%.1f]", player.DisplayName, newHealth) 
            end
        else
...
Toggles.ESPHighlight:OnChanged(function(value)
    for _, espType in pairs(Script.ESPTable) do
        for _, esp in pairs(espType) do 
            if esp.CurrentSettings then esp.CurrentSettings.Visible = value end
        end
    end
end)

Toggles.ESPTracer:OnChanged(function(value) 
    -- В новой библиотеке нет глобального Tracers.Set, нужно обновлять инстансы
    for _, espType in pairs(Script.ESPTable) do
        for _, esp in pairs(espType) do 
            if esp.CurrentSettings and esp.CurrentSettings.Tracer then
...
Toggles.ESPRainbow:OnChanged(function(value) 
    -- Глобальная настройка радуги
    if ESPLibrary.GlobalConfig then
        ESPLibrary.GlobalConfig.Rainbow = value
    end
end)

Options.ESPFillTransparency:OnChanged(function(value)
    for _, espType in pairs(Script.ESPTable) do for _, esp in pairs(espType) do 
        if esp.CurrentSettings then esp.CurrentSettings.FillTransparency = value end
    end end
end)

Options.ESPOutlineTransparency:OnChanged(function(value)
    for _, espType in pairs(Script.ESPTable) do for _, esp in pairs(espType) do 
        if esp.CurrentSettings then esp.CurrentSettings.OutlineTransparency = value end
    end end
end)

Options.ESPTextSize:OnChanged(function(value)
    for _, espType in pairs(Script.ESPTable) do for _, esp in pairs(espType) do 
        if esp.CurrentSettings then esp.CurrentSettings.TextSize = value end
    end end
end)

Options.ESPTracerStart:OnChanged(function(value)
    for _, espType in pairs(Script.ESPTable) do for _, esp in pairs(espType) do 
        if esp.CurrentSettings and esp.CurrentSettings.Tracer then esp.CurrentSettings.Tracer.From = value end
    end end
end)
...
local function UpdateColor(optionName, espTableKey)
    Options[optionName]:OnChanged(function(value)
        for _, esp in pairs(Script.ESPTable[espTableKey]) do
            if esp.CurrentSettings then
                esp.CurrentSettings.FillColor = value
                esp.CurrentSettings.OutlineColor = value
                esp.CurrentSettings.Color = value -- Для текста
                if esp.CurrentSettings.Tracer then esp.CurrentSettings.Tracer.Color = value end
            end
        end
    end)
end
...
ThemeManager:SetFolder("NexusESP_V3")
